...
def calc_FlipRF(m_RF, m_Dz, m_Nz, m_Gbg) -> RfWaveform:
    ...
    Uses a full non-linear Bloch simulation to accurately model the slice profile 
    edges for large tip angles (e.g., 90° and 180°), avoiding the small-angle approximation.
    ...
    # 2. Simulation de Bloch à pleine puissance (SANS l'approximation 1e-4)
    M_final = pulse_sim(M0, z_axis, m_RF, m_Gbg)
    
    # 3. Extraction rigoureuse de l'angle de bascule via l'aimantation transversale et longitudinale
    Mx = M_final[0, :]
    My = M_final[1, :]
    Mz = M_final[2, :]
    
    # Calcul de la composante transversale M_perp
    M_perp = np.sqrt(Mx**2 + My**2)
    
    # arctan2(y, x) gère correctement les angles au-delà de 90° et les signes
    alpha_rad = np.arctan2(M_perp, Mz)
    
    # Conversion en degrés
    m_RF.alpha = np.degrees(alpha_rad)

    logger.info("Spatial flip angle distribution (.alpha) successfully evaluated using full non-linear model.")
    return m_RF
...
def get_RF(m_RF, m_Nrf, m_Dz, m_Nz):
    ...
    Args:
        ...
        m_Nrf (int): Number of points for the interpolated waveform profile.
    ...
    if isinstance(loaded_rf.G, (list, np.ndarray)) and len(loaded_rf.G) > 1:
        loaded_rf.G = rfCons._interpolate_rf(loaded_rf.G, m_Nrf)
        
    # Apply the scaling to Gauss
